...
- 2026-06-06 Shop All smarter search pass keeps search as a lightweight finder rather than a heavy modal/dropdown. The search bar now has a left search mark, clearer placeholder copy (`Search color, occasion, style...`), an inline clear control while typing, result-count feedback, starter chips (`pink`, `bridal`, `vacation`, `simple`, `under $30`) while empty, broader matching across product name, description, collection, detail tier, popularity/newness, and price labels, plus a compact no-results state with `Clear search`.
