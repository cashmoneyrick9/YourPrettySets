...
- 2026-06-09 Product page selector polish upgrades Length and Shape into one consistent horizontal carousel tile system while preserving the same option state, button semantics, `aria-pressed` behavior, routing, product data, imagery, and add-to-cart behavior. Both groups use compact label-only tiles with CSS nail silhouettes, native horizontal scrolling, scroll snap, shared card sizing, shared selected styling, and the selected style summary now appears as a compact confirmation module instead of plain `Selected:` text.
...
- Product page Length and Shape selectors now share a consistent horizontal carousel tile system. The selected style summary was upgraded from plain `Selected:` text into a compact confirmation module showing the selected length and shape.
