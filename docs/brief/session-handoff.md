...
- 2026-07-08 mobile menu interaction polish keeps the split-screen design in the expanded state but debounces selector wheel gestures for 400ms so trackpads cannot rapidly toggle Shop/Help. The centered default menu gently enters on open, Shop/Help selection slides into the left selector position, and the right submenu panel replays one calm `mobile-submenu-enter` fade/8px slide animation when Shop/Help changes, with the animation disabled under `prefers-reduced-motion`. Follow-up CSS cleanup separates default and expanded layout ownership: default mode uses normal centered flow with no active indicator or offset transforms, while expanded mode alone owns the absolute wheel selector positioning. The menu selector surfaces now use the flat white `#fffdfb` surface, and the thin active selector indicator line is intentionally hidden.
- 2026-07-08 mobile menu transition fix keeps the blush overlay as the stable full-screen base and changes the off-white submenu from a grid-owned column to an absolute 62% right-side sheet. The default state leaves the sheet in the DOM but translated offscreen with `translateX(100%)`, pointer events disabled, and `aria-hidden`; the expanded state slides it to `translateX(0)` over 520ms while the left selector rail owns 38% width. Do not reintroduce a default-to-expanded `grid-template-columns` swap for this interaction; it caused the visible white-panel blink/jump. Reduced motion keeps the same default/expanded transform endpoints but removes the long transform transition.
...
- Mobile navigation is currently a two-state interaction with Home, Shop, and Help. Opening the menu starts in a centered default state with only the three main items visible on a flat white `#fffdfb` background; no parent item is active by default and no submenu panel is shown.
- Selecting Shop or Help transitions into the split-screen submenu state: the centered selector group shifts into the left rail, the left rail also uses the flat white `#fffdfb` surface without a separate vertical active indicator line, and the right content panel slides/fades in from the right. Shop reveals Ready to Ship, Made to Order, and Custom Orders; Help reveals the existing Help links. Shop/Help can still be changed with the active selector, wheel, or touch gestures after expansion.
