...
- 2026-07-09 Browse carousel unselect fix keeps the no-remount carousel behavior and the larger `.collection-track` gutters, but separates selected option state from the controlled scroll target. Browse still starts with `scrollToIndex: null` so auto-rotation can own the lane, selecting an option scrolls to that option, and clearing a selected option after any Browse interaction sends `scrollToIndex: 0` so Embla resets to the clean default position instead of keeping a stale selected offset. Unselecting still hides `.collection-products` and does not resume auto-rotation.
