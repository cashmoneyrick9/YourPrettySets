...
- 2026-06-27 Mobile Shop submenu visual refinement keeps the `Home`, `Shop`, and `Help` overlay structure, but removes the old Shop submenu container/divider treatment. `Shop` should read as the centered parent heading, with `Ready to Ship`, `Made to Order`, and `Custom Orders` as smaller muted centered subheading-style links underneath; do not reintroduce a boxed container, left rule, or left-offset submenu unless the founder reopens that direction.
