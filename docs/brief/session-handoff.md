...
- Selecting Shop or Help transitions into the split-screen submenu state: the centered selector group shifts into the left rail, the left rail uses the current `#eab6b4` blush panel, and the right content panel slides/fades in from the right. Shop reveals Ready to Ship, Made to Order, and Custom Orders; Help reveals the existing Help links. Shop/Help can still be changed with the active selector, wheel, or touch gestures after expansion.
