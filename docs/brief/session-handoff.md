...
- Mobile navigation is currently a two-state interaction with Home, Shop, and Help. Opening the menu starts in a centered default state with only the three main items visible; no parent item is active by default and no submenu panel is shown.
