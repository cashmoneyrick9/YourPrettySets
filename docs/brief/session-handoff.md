...
- Follow-up Product page selector refinement on 2026-06-09 keeps the same structure and state logic but improves nail silhouette proportions, softens the selected tile treatment, and keeps compact helper text visible on desktop while tightening only the buying-panel spacing.
- Follow-up Length selector mobile layout tweak on 2026-06-09 changes only the mobile Length options from a 2x2 grid into a horizontal card carousel with peeked option cards and native horizontal scrolling. Shape remains a grid for comparison, while desktop keeps the compact grid layout.
- Follow-up mobile Length selector compaction on 2026-06-09 removes Length helper subcopy, stacks each Length label under its nail icon, and makes the mobile Length carousel cards narrower and shorter. Shape helper copy and grid layout remain unchanged.
- Follow-up mobile Length selector width trim on 2026-06-09 narrows the Length carousel cards further so the row wastes less side space and can show all four Length options in a 467px mobile viewport while keeping the same icon-over-label structure.
- Follow-up mobile Length selector symmetry fix on 2026-06-09 replaces the fixed-width Length row with four equal mobile columns so the row is visually even and no longer leaves dead space at the right edge.
- Follow-up mobile Shape selector match on 2026-06-09 changes Shape to the same compact icon-over-label tile treatment as Length, removes Shape helper subcopy, and uses a symmetrical 3x2 mobile grid for the six shape options.
