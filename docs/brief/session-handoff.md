...
- Mobile navigation is currently a two-state interaction with Home, Shop, and Help. Opening the menu starts in a centered default state with only the three main items visible over a single translucent warm white `rgba(255, 253, 251, 0.62)` blurred dialog backdrop; no parent item is active by default and no submenu panel is shown.
