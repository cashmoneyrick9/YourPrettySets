...
- 2026-07-10 mobile menu close lifecycle refinement keeps the full dialog mounted, scroll-locked, and visually present for a 440ms opacity exit when X, Escape, Home, or a submenu destination closes it; only after that exit does it unmount, restore the page, and return focus to the menu control. `prefers-reduced-motion` skips that wait. Tapping the active Shop or Help parent immediately returns the selectors to their centered default positions and sends the white panel offscreen, but retains the outgoing submenu DOM until the existing 620ms sheet transition completes; it is then cleared. Keep these two temporary states intact rather than directly coupling rendered content or dialog visibility to the final closed state.
- 2026-07-10 mobile menu reverse-close correction supersedes the close timing above. The mobile menu now uses an explicit `collapsing` phase instead of switching directly from `expanded` to `default`: the selector rail animates from its rendered 38% width to 100%, Home/Shop/Help move back to their centered transform offsets, and the mounted white submenu panel translates from `0` to `100%` together over 620ms. The outgoing submenu is cleared only when that phase completes, and `mobile-menu-default-enter` is never applied during collapse. Full-menu closes from an expanded state run that same 620ms collapse first, then a 160ms overlay opacity exit, then unmount/unlock/restore focus; closes from the already-centered default state skip collapse and use only the 160ms exit. Reduced motion completes both phases immediately, and repeated close input is ignored while either phase is active.
- 2026-07-10 mobile menu close-blink fix preserves the completed `collapsing` render throughout the 160ms overlay fade. Beginning the fade now changes only the dialog-closing state: the active submenu ID, outgoing submenu DOM, `collapsing` menu/selector/content classes, and completed selector/panel positions remain frozen until the dialog is hidden. Internal state resets to a clean `default` menu only in a post-close effect after `isMenuOpen` has committed false. Submenu-only Shop/Help collapse still clears its submenu and returns the still-open overlay to default after 620ms.
- 2026-07-11 mobile menu compositing-blink fix supersedes the final 160ms fade above. The blurred/translucent dialog no longer animates opacity. Expanded full-menu closes keep the overlay fully visible through the 620ms reverse collapse, synchronously restore body/root scroll styles and the saved scroll position while the overlay still covers the viewport, allow one animation frame for the restored page to paint behind that opaque overlay, then hide the dialog on the following frame. Default centered closes perform the same synchronous unlock plus two-frame paint handoff without waiting 620ms. This two-frame boundary prevents the transient blank compositor frame observed with a single-frame hide. Internal submenu/mode cleanup still happens only after the dialog is hidden, reduced motion hides immediately after synchronous unlock, and submenu-only Shop/Help collapse remains unchanged.
- 2026-07-17 mobile menu expanded-close selector fix supersedes the reverse-selector portion of the full-menu close above. Closing the full overlay from an expanded Shop or Help submenu now keeps the selector rail at 38% with the active label, inactive label styling, and active-relative offsets frozen while only the right submenu sheet translates offscreen over 620ms. The page then uses the existing two-frame unlock/hide handoff and resets to default only after the dialog is hidden. This applies consistently to X, Escape, Home, and submenu-destination closes. Tapping the active Shop or Help label is intentionally separate: it still uses the `collapsing` state to animate the rail and all three labels back to the centered default menu without closing the overlay.
- 2026-07-17 mobile menu selector compositing correction fixes the remaining rendered label blink that state/class checks did not catch. While the sibling submenu sheet transformed offscreen, the browser could temporarily drop the inactive Home/Help text layers even though their classes, size, visibility, and offsets stayed stable; the full-opacity active Shop label remained rendered. All three selector items now use equivalent `translate3d(...)` positioning with hidden backfaces and persistent transform layers, and the inactive selector items stay at full element opacity so the compositor cannot discard their close frames. Their existing muted color still preserves the inactive hierarchy. Keep the current offsets, typography, expanded mask/wheel cue, and active-parent submenu-only collapse behavior.
- 2026-07-17 mobile menu route-scroll fix keeps that close animation and paint handoff intact but delays ordinary internal navigation until the overlay has fully hidden and released its saved scroll position. Mobile Home and submenu links prevent only unmodified primary-click navigation, call the existing close callback, then use React Router to change routes; modified clicks keep normal browser behavior. Same-route selections still close the menu without forcing a scroll reset, while real route changes now run `ScrollToTop` against the unlocked document instead of having the old page offset restored over the destination. Integration coverage opens all 11 mobile destinations from a simulated deep Shop scroll, and browser verification at `393x852` confirmed `/shop/made-to-order` stayed active during the close before `/shop/ready-to-ship` settled at `scrollY: 0` with its H1 at `112px`.
