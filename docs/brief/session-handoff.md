...
- 2026-07-17 mobile menu close-mask correction fixes the remaining rendered label blink that state/class checks did not catch. The expanded selector's top/bottom `mask-image` could be recomposited while the sibling submenu sheet transformed offscreen, temporarily dropping the inactive Home/Help labels even though their classes, size, opacity, and transforms stayed stable. Full-overlay close now disables only that selector mask while `brand-header__mobile-menu--closing` is active; the open expanded menu keeps its normal mask/wheel cue, selector geometry and typography stay unchanged, and active-parent submenu-only collapse remains unchanged.
