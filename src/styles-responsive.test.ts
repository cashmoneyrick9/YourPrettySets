...
describe("small mobile responsive CSS", () => {
  ...
  it("maps homepage headings, buttons, and nav to typography roles", () => {
    ...
    expect(styles).toContain("--mobile-menu-default-type: clamp(2.1rem, 11vw, 3.35rem);");
    expect(styles).toContain("--mobile-menu-active-type: clamp(2rem, 9.8vw, 3.1rem);");
    expect(styles).toContain("--mobile-menu-inactive-type: clamp(1.38rem, 6.8vw, 2.05rem);");
  ...
  it("keeps the approved footer mobile-first and tokenized", () => {
    ...
    expect(styles).toContain('background-image: url("/assets/footer-email-sets.png");');
    expect(styles).toContain("filter: brightness(0.88) saturate(0.86);");
    ...
    expect(styles).toContain("background: rgba(16, 20, 23, 0.68);");
    ...
    expect(styles).toContain(".site-footer-email-shell + .site-footer {\n    --footer-link-size: 0.78rem;\n    --footer-section-label-size: 0.84rem;\n    padding-top: 46px;");
    expect(styles).toContain(".site-footer__inner {\n    gap: 28px;\n    padding-inline: 16px;");
    expect(styles).toContain(".site-footer__brand-block {\n    gap: 5px;");
    expect(styles).toContain("@media (max-width: 720px)");
    expect(styles).toContain(".site-footer__columns {\n    column-gap: clamp(8px, 3vw, 16px);\n    grid-template-columns: repeat(3, minmax(0, 1fr));\n    justify-self: center;\n    max-width: 420px;\n    width: 100%;");
    expect(styles).not.toContain(".site-footer__column:last-child");
    expect(styles).toContain(".site-footer__columns {\n    column-gap: clamp(8px, 3vw, 16px);\n    width: 100%;");
    expect(styles).toContain(".site-footer__column {\n    gap: 12px;");
    expect(styles).toContain(".site-footer__column-title {\n  color: var(--footer-accent);\n  font-family: var(--font-cta);\n  font-size: var(--footer-section-label-size);\n  font-weight: 820;\n  letter-spacing: 0.1em;");
  ...
  it("keeps mobile selector items in one transform-based layout across menu states", () => {
    ...
    expect(rootSelectorItemRule).toContain('font-variation-settings: "opsz" 144, "SOFT" 54, "WONK" 0;');
    expect(rootSelectorItemRule).toContain("font-size: var(--mobile-menu-inactive-type);");
    expect(rootSelectorItemRule).toContain("letter-spacing: var(--mobile-menu-inactive-tracking);");
    expect(rootSelectorItemRule).toContain("letter-spacing 420ms ease,");
    expect(styles).toContain(
    ...
    expect(activeSelectorItemRule).toContain("font-family: var(--font-display);");
    expect(activeSelectorItemRule).toContain('font-variation-settings: "opsz" 144, "SOFT" 54, "WONK" 0;');
    expect(activeSelectorItemRule).toContain("font-size: var(--mobile-menu-active-type);");
    expect(activeSelectorItemRule).toContain("letter-spacing: var(--mobile-menu-active-tracking);");
    ...
    expect(styles).toContain(
      ".brand-header__mobile-menu--expanded .brand-header__mobile-selector-item,\n  .brand-header__mobile-menu--closing-panel .brand-header__mobile-selector-item,\n  .brand-header__mobile-menu--returning-selector .brand-header__mobile-selector-item"
    );
    ...
    expect(defaultContentRule).toContain("opacity: 1;");
    ...
    expect(defaultContentRule).toContain("visibility: visible;");
  ...
  it("uses transform-based mobile submenu sheet motion instead of grid ownership changes", () => {
    const defaultMenuRule = getRuleBody(
      styles,
      ".brand-header--menu-open .brand-header__mobile-menu--default"
    );
    const expandedMenuRule = getRuleBody(
      styles,
    ...
    const selectorClosingPanelRule = getRuleBody(styles, ".brand-header__mobile-menu-selector--closing-panel");
    const selectorReturningRule = getRuleBody(styles, ".brand-header__mobile-menu-selector--returning-selector");
    const contentRule = getRuleBody(styles, ".brand-header__mobile-menu-content");
    const menuDialogRule = getRuleBody(styles, ".brand-header--menu-open .brand-header__mobile-menu-dialog");
    const defaultContentRule = getRuleBody(styles, ".brand-header__mobile-menu-content--default");
    const expandedContentRule = getRuleBody(styles, ".brand-header__mobile-menu-content--expanded");
    ...
    expect(selectorClosingPanelRule).not.toContain("animation: mobile-menu-default-enter");
    expect(selectorReturningRule).not.toContain("animation: mobile-menu-default-enter");
    ...
    expect(styles).not.toMatch(/\.brand-header__mobile-menu--closing(?:\s|\.|\{|,)/);
    expect(defaultContentRule).toContain("transform: translateX(100%);");
  ...
  it("keeps the mobile selector motion calm and respects reduced motion", () => {
    ...
    expect(styles).toContain(
      ".brand-header__mobile-menu-selector--default.brand-header__mobile-menu-selector--skip-enter {\n    animation: none;"
    );
