...
describe("ProductCard", () => {
  it("shows the catalog-owned product image without visible placeholder copy", () => {
    render(<ProductCard product={products[0]} />);

    expect(screen.getByText(products[0].name)).toBeInTheDocument();
    expect(screen.getByText("$18")).toBeInTheDocument();
    expect(screen.getByRole("img", { name: products[0].images.clean })).toHaveAttribute(
      "src",
      products[0].media?.clean
    );
    expect(screen.queryByText(products[0].images.clean)).not.toBeInTheDocument();
    expect(document.querySelectorAll(".product-art__nail")).toHaveLength(0);
